...
def md_json_converter():
    """
        ...
        file sorts each image JSON objects into their according "Location", "Station", "Session", and
        "Predicted Category". The sorting is done primarily based on the storing location path of each image in the
        output JSON file from MegaDetector batch processing.

        When execute the function, it will first prompt the user to enter the absolute path of the output JSON file
        from MegaDetector batch processing. Then it will ask the user to give a name and an absolute path for the
        new JSON file.

        :param:
            usr_input_json: the absolute path of the output JSON file from MegaDetector batch processing
            (end with '.json')
...
def cropper_metadata_creator():
    """
        ...
        creates a dataframe by calling json_extractor() and update the "Image Path" column with the rightful paths for
        the image. The updated dataframe will then be saved in a CSV classified_metadata file (usr_output_csv)
...
def cropper():
    ...
    input_usr_path = input("Enter the absolute path of the image that you would like to perform the cropping on: ")

    output_usr_path = input("Enter the absolute path of where you would like the cropped image to be saved "
                            "(end with '/'): ")

    input_usr_csv = input("Enter the absolute path of where the '*_MetaCropped.csv' "
                          "file is located in (end with '.csv'): ")

    usr_input_path = input_usr_path.replace("\\", "/") + "/"
    usr_output_path = output_usr_path.replace("\\", "/") + "/"
    usr_input_csv = input_usr_csv.replace("\\", "/") + "/"
